# Current sourcebound product contract

<!-- sourcebound:policy register-v2 -->
<!-- sourcebound:purpose -->
Repository maintainers use this contract to decide what a green sourcebound result actually
...
`sourcebound verify` is the proof: its receipt names the configured assurance scope and reports
direct bindings separately from cataloged surface.

## What sourcebound is

sourcebound is a local documentation-control engine and CLI. It binds selected documentation facts
...
<!-- sourcebound:begin assurance-boundaries -->
| surface | sourcebound proves | sourcebound does not prove |
| --- | --- | --- |
...
checked `<!-- sourcebound:role reference -->` marker; invalid role names and unclosed opening
...
together. The contract is observe-only. sourcebound compares locator digests at two immutable refs;
...
Use the [read-only verification skill](skills/sourcebound-verify/SKILL.md) when an external agent
...
bounded local `sourcebound.feedback.v1` envelopes; they still make no network request. `preview`
...
An external controller may return an aggregate `sourcebound.behavior-signal.v1`. Ingest records it
...
The canonical file is `.sourcebound.yml`. Init writes manifest version `2`; version `1` remains a
...
Live evaluation writes `sourcebound.provider-run.v1` before it invokes a command provider. The
